...
You will also need to add some other asset management gems:
...
and visiting localhost:3000/records/<document number> in your browser. If you see an error about not finding a class, try adding <tt>config.cache_store = :null_store</tt> to your +config/environments/development.rb+ file.
...
To load the VirtualShelf application in a similar manner for your catalog, add the following line to the appropriate template file:
  ...
  <script src="<your site's url>/virtual_shelf/catalog/load.js" type="text/javascript"></script>
